...
void ShellProc()
{
	...
	MyStrcpy(my_msg.data, "Hello World Team MIOS\n");
	...
	while(1)	//forever loop:
	{
		//prompt:
			...
			MyStrcpy(my_msg.data, "Enter a command: ");// MyStrcpy what's to be prompted to my_msg.data--hello world team Mios
...
//prompt "just entered -> " onto terminal( same details as above prompting example)	
			my_msg.recipient = port_data.stdin_pid; 	// set recipient to StdoutProc
			MsgSnd(&my_msg);			// send my_msg
			MsgRcv(&my_msg);		//receive my_msg ( a reply, contetent don't care)
			MyStrcpy(a_string, my_msg.data);	//copy my_msg.data to a_string (save for later)
