...
void ShellProc()
{
	...
	TX_semaphore = msg_q[MAX_PROC_NUM]			// request for a semaphore as TX_semaphore, limit is max queue length*******************
	...
	while(1)//forever loop:********************
	{
		//prompt:
			...
			MyStrcpy((char *) &my_msg.data, (char *) &completion);// MyStrcpy what's to be prompted to my_msg.data
			temp_msg.recipient = StdoutProc;	// set recipient to StdoutProc
			MsgSnd(&my_msg);			// send my_msg
...
// show a_string onto terminal (same details as above promppting example)
		...
		// get whats entered:
			temp_msg.recipient = StdinProc;		// set recipient to StdinProc
			MsgSnd(&my_msg);			// send my_msg
...
void StdinProc()
{
	...
	p = my_msg.data;			//char ptr p points to my_msg.data
		...
		while(1) //**************
		{
			SemWait(RX_semaphore); 		// semaphore wait on RX_semaphore of port data*************
			outportb(COM2_IOBASE+_IER, IER_ERXRDY|IER_ETXRDY);		
			ch = MsgDeQ(&port_data.RX);	// ch = dequeue frm RX_buffer of port data**********
...
void StdoutProc()
{
	...
	while(1)	// forever loop:
	{
		...
		p = my_msg.data;
			...
			while(p !='\0')
			{
				SemWait(TX_semaphore);// semaphore wait on TX_semaphore of port data ****
				outportb(COM2_IOBASE+_IER, IER_ERXRDY|IER_ETXRDY);				
