...
/*
//phase 6 //basically all comments step 6 later half

void IRQ3ISR()
{
	int dismiss; 		
	dismiss = inportb(COM2_IOBASE+_IIR);	// dismiss irq3
...
void RX() 
{
	...
	ch = inportb(COM2_IOASE+DATA) & 07F;	//mask 0111 1111

	MsgEnQ(ch, &portdata.RX)			//enqueue ch to RX buffer of port data******
	SemPostISR(&port_data.RX_semaphore);		//SemPostISR RX_semaphore of port data******

	if(ch == '\r')
	{
		//enqueue '\r' then '\n' to ech buffer of port data*********
