...
//////////////////////////phase 8
char take5_bin[]= {
   #include "executables/take5.bin" // this must be separate line
};
#define TAKE5_SIZE (sizeof(take5_bin))

char say_hello_bin[]= {
   #include "executables/say_hello.bin" // this must be separate line
};
#define SAYHELLO_SIZE (sizeof(say_hello_bin))
...
dir_t bin_dir[] = {
   ...
   { 18, MODE_EXEC, TAKE5_SIZE, "take5", (char *)take5_bin },
   { 19, MODE_EXEC, SAYHELLO_SIZE, "say_hello", (char *)say_hello_bin },
...
// if name ends in '/', chances are we need to decend into the dir
   ...
   for( ; dir_p->name; dir_p++ ) {
      if((unsigned int)dir_p->name > 0xdfffff) return NULL;
